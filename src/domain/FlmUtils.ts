...
export function decodeFLMvideo(
  ...
  // We'll preserve starting pointer for final consumed calculation
  const startPu = puVar8_byte;
  ...
  while (local_24 !== 0) {
    ...
    while (local_28 !== 0) {
      ...
      let uVar1 = readU32(puVar8_byte);

      if (local_1c === 0) {
        const uVar6 = uVar1 & 0x1f; // low 5 bits used to decide action
        let uVar2 = readI32(puVar8_byte + 4); // puVar8[1] read as signed (int) in some tests
