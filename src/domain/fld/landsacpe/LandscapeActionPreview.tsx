...
const Preview = (props: PreviewProps) => {
  ...
  const width = useMemo(
    () => new Set(points.map((p) => p.index % fldFile.width)).size,
    [points, fldFile.width],
  );
  const height = useMemo(
    () => new Set(points.map((p) => fldFile.height - 1 - Math.floor(p.index / fldFile.width))).size,
    [points, fldFile.height, fldFile.width],
  );
