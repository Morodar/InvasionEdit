...
export class LevelPckParser {
  static async parseLevelPck(file: File): Promise<LevelPck> {
    ...
    const { levelEntries: cgnLevelEntries } =
      await extractCampaignLevelEntriesAsLevelEntries(pckFiles);
    ...
    const { levelToFld, usedEntries: usedFldEntries } = await extractFldFiles(
      pckFiles,
      levelNameToLevelEntry,
    );
    usedFiles.push(...usedFldEntries);

    // based on level entries, we also extract lev files
    const { levelToLev, usedEntries: usedLevEntries } = await extractLevFiles(
      pckFiles,
      levelNameToLevelEntry,
    );
...
function determineRemainingFiles(
  allFiles: PckFileEntry[],
  usedFiles: PckFileEntry[],
): PckFileEntry[] {
...
async function extractCampaignLevelEntriesAsLevelEntries(
  pckEntries: PckFileEntry[],
): Promise<LevelEntryResult> {
  const levels: LevelEntry[] = [];
