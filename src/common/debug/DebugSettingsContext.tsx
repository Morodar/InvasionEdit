...
export const DebugSettingsContextProvider: React.FC<PropsWithChildren> = ({
  children,
}: PropsWithChildren) => {
  ...
  const setShowAllLayers = (value: boolean) =>
    setSettings((old) => ({ ...old, showAllLayers: value }));
  const setShowDebugCube = (value: boolean) =>
    setSettings((old) => ({ ...old, showDebugCube: value }));
  const setShowDebugCube3x3 = (value: boolean) =>
    setSettings((old) => ({ ...old, showDebugCube3x3: value }));
  const setShowEntitiesList = (value: boolean) =>
    setSettings((old) => ({ ...old, showEntitiesList: value }));
  ...
  return (
    <DebugSettingsContext.Provider value={contextValue}>{children}</DebugSettingsContext.Provider>
  );
};
